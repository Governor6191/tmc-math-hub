...
(async () => {
  ...
  try {
    const res = await fetch(`data/questions/${courseId}/drafts/${topicId}.json`);
    ...
    root.innerHTML = `
      <h1>Draft review: ${escapeHtml(courseId)} / ${escapeHtml(topicId)}</h1>
      <div class="draft-banner">Internal page. ${bank.questions.length} draft questions
        (${counts.warmup} warmup, ${counts.core} core, ${counts.stretch} stretch). Status: ${escapeHtml(bank.status)}.
        Correct answers are highlighted. To publish: tell Claude to promote this bank, or run
        node tools/promote-bank.js ${escapeHtml(courseId)} ${escapeHtml(topicId)}</div>
